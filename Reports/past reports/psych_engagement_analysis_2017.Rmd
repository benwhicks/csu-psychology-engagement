...
The following data is from the Social Psychology (PSY203 and PSY453) and Biopsychology (PSY208 and PSY408) subjects run in 2017 Session 2. The aim of this report is to look at student engagement in these courses through the data available in Blackboard Learn. 
...
BIOAAFILE <- "aa Biopsychology 201760.csv"
SOCAAFILE <- "aa Social Psychology 201760.csv"
MARKSFILE <- "marks Psychology 201760.csv"
BIOCCFILE <- "cc Biopsychology 201760.csv"
SOCCCFILE <- "cc Social Psychology 201760.csv"
POSTSFILE <- "posts Psychology 201760.csv"
...
aaSoc17 <- read_csv(file.path(DATDIR, SOCAAFILE), col_types = "ffTcccccffcccf") # can cause performance hit, if so do not specity columns and add as.factor lines to tidy_aa 
aaSoc17 <- tidy_aa(aaSoc17)

aaBio17 <- read_csv(file.path(DATDIR, BIOAAFILE), col_types = "ffTcccccffcccf")
aaBio17 <- tidy_aa(aaBio17)

# Node preparation (i.e. data for each student)

# Mark data
marks17 <- read_csv(file.path(DATDIR, MARKSFILE)) %>%
  mutate(mark = score / possible, id = factor(id)) 
marksSummary17 <- marks17 %>% 
  group_by(subject_name, student, id) %>%
  summarise(avg_grade = mean(mark, na.rm = T)) %>%
  rename(name = student, subject = subject_name)
marksSoc17 <- marksSummary17 %>% filter(subject == "Social Psychology")
marksBio17 <- marksSummary17 %>% filter(subject == "Biopsychology")
...
discussionViewsBio <- discussion_views(aaBio17)
discussionViewsSoc <- discussion_views(aaSoc17)

# Other interaction data from aa
aaSummaryBio <- aa_summary_by_user(aaBio17)
aaSummaryBio <- merge(aaSummaryBio, unique(aaBio17 %>% select(id, name)))
aaSummarySoc <- aa_summary_by_user(aaSoc17)
aaSummarySoc <- merge(aaSummarySoc, unique(aaSoc17 %>% select(id, name)))
...
nodesBio <- node_merge_and_make(aaSummaryBio, marksBio17, discussionViewsBio)
nodesSoc <- node_merge_and_make(aaSummarySoc, marksSoc17, discussionViewsSoc)
...
## ADD ##
allStudentIDs <- unique(marks17$id)
...
socUsage <- aaSoc17 %>%
  ...
  filter(role == "S" & timestamp < as.Date('2017-11-01')) %>%
...
socForumExamine <- aaSoc17 %>% filter(role == "S") %>% select(id, timestamp, content, forum_bool) %>%
...
bioGameActPlotFocus <- bioUsage %>% filter(timestamp < as.Date('2017-10-15')) %>%
  ggplot(aes(x = timestamp, y = grade_quartile, color = ForumViews)) +
  scale_color_gradient2(name = "Forum Participation", 
                         low = "Black",
                          mid = "Dark Blue",
