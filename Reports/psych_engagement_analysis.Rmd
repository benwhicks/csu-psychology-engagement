...
nodes_from_cc <- function(x) {
  x %>% select(id = pk1, title)
  # might need to impute cluster, qual and grade_quartile fields
}
...
socSiteGraphList <- build_site_activity_graph_list(
  cc_nodes = nodes_from_cc(ccSoc18),
...
draw_site_activity_graph(socSiteGraphList, alpha_low = 0.05) + ggtitle("Social Psychology activity of qual students")
...
bioSiteGraphList <- build_site_activity_graph_list(
  cc_nodes = nodes_from_cc(ccBio18),
...
draw_site_activity_graph(bioSiteGraphList, alpha_low = 0.05) + ggtitle("Biopsychology activity of qual students")

```


``` {r Biopsych_forum_access, eval = F, cahced = TRUE}
...
``` {r animated_activity_of_clusters}
```
